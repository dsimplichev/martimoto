...
function AddCarTiresPage() {
    ...
    const handleChange = (e) => {
        ...
        const val = (name === 'width' || name === 'aspectRatio' || name === 'noiseLevel' || name === 'price') 
                    ? Number(value) : value;
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in tireData) {
            if (key !== 'images' && tireData[key] !== null) { 
                formData.append(key, tireData[key]);
            }
        }
        
        
        tireData.images.forEach((image) => {
            formData.append('images', image); 
        });

        try {
            
            const response = await axios.post('http://localhost:5000/api/admin/add-car-tire', formData, {
                headers: {
            ...
            <form onSubmit={handleSubmit} className="add-tire-form">
                ...
                <div className="form-row">
                    <input 
                        type="text" 
                        name="brand" 
                        placeholder="Марка (напр. Michelin)" 
                    ...
                    <input 
                        type="text" 
                        name="model" 
                        placeholder="Модел (напр. Pilot Sport 4)" 
                ...
                <div className="form-row">
                    <input 
                        type="text" 
                        name="loadIndex" 
                        placeholder="Товарен индекс (напр. 91)" 
                    ...
                    <input 
                        type="text" 
                        name="speedRating" 
                        placeholder="Скоростен индекс (напр. V)" 
                    ...
                    <input 
                        type="text" 
                        name="fuelEconomy" 
                        placeholder="Икономичен индекс (напр. A)" 
                        value={tireData.fuelEconomy} 
                        onChange={handleChange} 
                        required 
                    />
                </div>
                ...
                <div className="form-row">
                    <input 
                        type="text" 
                        name="wetGrip" 
                        placeholder="Сцепление на мокро (напр. B)" 
                        value={tireData.wetGrip} 
                        onChange={handleChange} 
                        required 
                    />
                    <input 
                        type="number" 
                        name="noiseLevel" 
                        placeholder="Индекс шум (напр. 72)" 
                        value={tireData.noiseLevel} 
                        onChange={handleChange} 
                        required 
                    />
                     <select 
                ...
                <textarea
                    name="description"
                    placeholder="Подробно описание на гумата"
                    value={tireData.description}
                    onChange={handleChange}
                    rows="4"
                ></textarea>
